...
namespace FaceRecognitionDotNet.Extensions.SkLearn.Utils
{
    ...
    public class Validation
    {
        ...
        public static void column_or_1d(y, bool warn = false)
        {
            ...
            throw new Exception("bad input shape {0}".format(shape))
        }

        public static check_X_y(X, y, accept_sparse= False, accept_large_sparse= True,
        ...
        private static void _num_samples(x)
        {
            ...
            if (!hasattr(x, "__len__") && !hasattr(x, "shape"))
                if hasattr(x, '__array__')
                    ...
                    throw new Exception($"Expected sequence or array-like, got {type(x)}");
...
# Dask dataframes may not return numeric shape[0] value
        ...
        public static void check_consistent_length(*arrays)
        {
            ...
            // """
            var lengths = [_num_samples(X) for X in arrays if X is not None]
        ...
        public static void check_array(array, bool accept_sparse = false, bool accept_large_sparse= true,
                        string dtype= "numeric", order= None, bool copy = false, bool force_all_finite = true,
                        bool ensure_2d = true, bool allow_nd = false, ensure_min_samples= 1,
                        ensure_min_features= 1, bool warn_on_dtype = false, estimator= None)
        {
            ...
            if (isinstance(dtype, (list, tuple)))
                if (dtype_orig != null && dtype_orig in dtype)
            ...
            if (warn_on_dtype && dtypes_orig != null && { array.dtype} != set(dtypes_orig))
            {
